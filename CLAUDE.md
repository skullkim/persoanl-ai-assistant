...
- 하드웨어 환경: Mac mini M4 Pro (14코어 CPU, 20코어 GPU, 48GB 통합 메모리, 1TB SSD).
- 핵심 제약: 모든 구성 요소(Backend, DB, LLM 엔진)는 단일 Mac mini 내부에서 로컬로 구동됨.
...
- 데이터베이스: PostgreSQL (Vector DB 확장을 위한 pgvector 또는 로컬 ChromaDB 병행)
...
5. 데이터 스키마 참조
- 투자 분석 결과와 원문 데이터를 연결할 수 있는 관계형 구조 유지.
